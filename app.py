...
class external:
    def move_file():
        ...
        if file_utils_.detect_files(filepath=get_external_inputpath):
            total_size, fileslist = file_utils_.folder_size(
                filepath=get_external_inputpath)

            zf, zipfileslist = zip_file_.zip_file(
                set_external_outputpath=set_external_outputpath, fileslist=fileslist)
            if zf is None:
...
class internal:
    def move_file():
        ...
        if file_utils_.detect_files(filepath=get_internal_inputpath):
            ...
            # 取得檔案表
            total_size, fileslist = file_utils_.folder_size(
                filepath=get_internal_inputpath)

            # 篩選檔案
            newsfileslist, ofileslist = file_utils_.internal_filter(
                fileslist=fileslist)
            ...
            # 解壓縮檔案
            zip_file_.unzip_file(
                set_internal_outputpath=set_internal_outputpath)
