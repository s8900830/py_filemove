...
class zip_file:
    ...
    def zip_file(zipname=f"input_{dtnow.strftime('%Y-%m-%d-%H-%M-%S')}", set_external_outputpath='', fileslist={}):
        try:
            part_num, zsize = 1, 0
            zipfiles, zipfileslist = [], []
            zip_temp_folder = set_external_outputpath

            # 壓縮檔案
            zf = ZipFile(
                f'{zip_temp_folder}\{zipname}_{part_num}.zip', mode="w")
            for name, fsize in fileslist.items():
                ...
                if zsize > size_change.size(config.ZIPFILE_SINGLE_SIZE):
                    ...
                    log.log(
                        f'{zip_temp_folder}\{zipname}_{part_num}.zip---成功執行檔案壓縮')

                    # 建了一個新的壓縮檔
                    zipfileslist.append(
                        f'{zip_temp_folder}\{zipname}_{part_num}.zip')
        ...
        try:
            for x in os.listdir(temppath):
                ...
                shutil.unpack_archive(
                    f'{temppath}\{x}', extract_dir=set_internal_outputpath)
            return True
