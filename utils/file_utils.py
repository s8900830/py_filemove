...
class utils: 
    ...
    # 內部檔案移動用
    def internal_move(set_internal_outputpath):
        try:
            list = os.walk(f'{set_internal_outputpath}\code')
            for root,dirs,files in list :
                for name in files:
                    # 這段還有問題要處理，檔案不會回到IDOLDATA這個階層
                    shutil.move(os.path.join(root,name),os.path.join(root.replace('\code\file_move\recreate\pre_input',''),name))'' 
